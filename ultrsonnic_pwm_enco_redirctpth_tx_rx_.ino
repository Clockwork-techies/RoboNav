/*
ESP32-based robot controller that receives movement commands via Bluetooth and executes them using motors, encoders, and ultrasonic sensors. 

Command Protocol :
Commands are 4-bit codes:
0x08: Move Forward
0x01: Move Backward
0x02: Turn Left
0x04: Turn Right
...
Response:
Sends a 3-bit wall status byte
Sends 0xFF if blocked by obstacle
...
void executeCommands() {
  for (commandIndex = 0; commandIndex < totalCommands; commandIndex++) {
    ...
    // Only skip wall check before first command
    ...
    skipInitialWallCheck = false; // Reset after first command

    // Execute movement
    if (cmd == 0x08) moveForDistance(DISTANCE_CM_TO_TRAVEL);
    else if (cmd == 0x04) turnRightAngle(90);
    else if (cmd == 0x02) turnLeftAngle(90);
    else if (cmd == 0x01) moveBackwardForDistance(DISTANCE_CM_TO_TRAVEL);

    // Wall check after movement
    long dist = readDistanceCM();
    if (dist > 0 && dist < WALL_DISTANCE_CM) {
    ...
    delay(300); // pause between commands
  }
...
void sendWallStatus() {
  uint8_t position = commandIndex + 1; // 1-based index
